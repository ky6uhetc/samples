...
import org.example.ru.futurio.command.Turn
...
class TurnableTest {
    ...
    fun `horizontal turn`() {
        ...
        Turn(spaceShip, Rotation(90.0, Z)).execute()
        assertEquals(Positioning(-0.0, -3.0), spaceShip.velocity)
        Turn(spaceShip, Rotation(60.0, Z)).execute()
    ...
    fun `vertical turn`() {
        ...
        Turn(spaceShip1, Rotation(30.0, Y)).execute()
        ...
        Turn(spaceShip2, Rotation(45.0, X)).execute()
        assertEquals(Positioning(-2.598076211353317, 0.0, -1.5), spaceShip1.velocity)
    }
    ...
    fun `can't turn object with undefined velocity`() {
        ...
        assertThrows<IllegalStateException> {  Turn(spaceShip, Rotation(30.0, Y)).execute() }.let { ex ->
    ...
    fun `can't turn object with undefined rotation`() {
        ...
        assertThrows<IllegalStateException> {  Turn(spaceShip, null).execute() }.let { ex ->
